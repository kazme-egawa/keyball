...
const uint16_t PROGMEM keymaps[][MATRIX_ROWS][MATRIX_COLS] = {
  [_QWERTY] = LAYOUT_universal(
    ...
    KC_LALT  , KC_LGUI  , KC_SPC   , KC_SPC   , KC_LGUI  , MO(_UTIL), KC_SPC   ,              KC_ENT , KC_BSLS  , _______  , _______  , _______  , KC_GRV   , SCRL_MO 
  ),
  ...
  [_UTIL] = LAYOUT_universal(
    ...
    KC_TAB   , KC_Q     , S(KC_DEL), KC_W     , C(KC_K)  , SCRL_DVD ,                                  SCRL_DVI , KC_LBRC  , LSFT(KC_LBRC),LSFT(KC_RBRC),KC_RBRC,_______,
    ...
    KC_LALT  , KC_LGUI  , KC_SPC   , KC_SPC   , KC_LGUI  , _______  , KC_SPC   ,            KC_ENT   , KC_DEL   , _______  , _______  , _______  , _______  , SCRL_MO
  ),
...
{
  if (report.x > 1 || report.x < -1 || report.y > 1 || report.y < -1 || report.h > 1 || report.h < -1 || report.v > 1 || report.v < -1)
  {
    layer_on(_AUTO_MOUSE);
    if (mouse_layer_token == INVALID_DEFERRED_TOKEN) {
      mouse_layer_token = defer_exec(MOUSE_TIMEOUT, mouse_layer_cb, NULL);
    } else {
      extend_deferred_exec(mouse_layer_token, MOUSE_TIMEOUT);
    }
  }
